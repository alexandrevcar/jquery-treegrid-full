...
(function ($) {
	...
	var methods = {
		...
		search: function (value) {
			...
			this.find('>tbody>tr').each(function () {
				...
				if (value.length && $(this).text().toLowerCase().indexOf(value) > -1) {
					$(this).toggle(true);
					if (_getChildNodes($(this)).length > 0) {
						_expand($(this));
					}
		...
		expandChild: function ($this) {
			if (_getChildNodes($this).length > 0) {
				_expand($this);
			}
		},
		collapseChild: function ($this) {
			if (_getChildNodes($this).length > 0) {
				_collapse($this);
			}
		},
	...
	function _parentExpander($this) {
		if (_getParentId($this) === null) return false;
		var $parent = _getParent($this);
		if (!_isExpanded($parent)) {
			$parent.toggle(true);
			_expand($parent);
		}
		return _parentExpander($parent);
	};
	function _parentCollapse($this) {
		if (_getParentId($this) === null) return false;
		var $parent = _getParent($this);
